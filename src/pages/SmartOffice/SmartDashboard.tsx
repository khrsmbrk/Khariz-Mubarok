      ...
      <div className="w-full md:w-64 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col">
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="font-bold text-slate-900 text-lg leading-tight">SMART</h1>
              <h2 className="text-blue-600 text-xs font-semibold tracking-wider">OFFICE RSML</h2>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-slate-200">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-slate-900 font-medium text-sm">{session.name}</p>
              <p className="text-slate-500 text-xs capitalize">{session.role}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-200">
          <button 
            ...
            className="w-full flex items-center px-4 py-3 text-red-600 hover:bg-red-50 rounded-lg transition text-sm font-medium"
          >
      ...
      <div className="flex-grow overflow-y-auto bg-slate-50">
        <header className="bg-white border-b border-slate-200 p-4 sm:p-6 flex justify-between items-center sticky top-0 z-10">
          ...
          <div className="flex items-center space-x-4">
            <Link to="/" className="text-sm text-slate-500 hover:text-slate-900 transition">Kembali ke Website</Link>
          </div>
        </header>
        ...
        <main className="p-4 sm:p-6 lg:p-8">
          {activeTab === 'dashboard' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-slate-900">Ringkasan Eksekutif</h3>
                <select 
                  value={timeFilter}
                  onChange={(e) => setTimeFilter(e.target.value as 'today' | 'past_7_days')}
                  className="bg-white border border-slate-200 text-slate-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 outline-none shadow-sm"
                >
              ...
              {session.role === 'management' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
                    ...
                    <p className="text-slate-500 text-sm font-medium mb-1">Bed Occupancy Rate (BOR)</p>
                    <div className="flex items-end justify-between">
                      <h4 className="text-3xl font-bold text-slate-900">{data.bor}</h4>
                      <span className="flex items-center text-emerald-600 text-sm font-medium"><TrendingUp className="w-4 h-4 mr-1" /> {data.borTrend}</span>
                    </div>
                  </div>
                  
                  <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><Users className="w-16 h-16 text-slate-900" /></div>
                    <p className="text-slate-500 text-sm font-medium mb-1">Pasien Rawat Jalan</p>
                    <div className="flex items-end justify-between">
                      <h4 className="text-3xl font-bold text-slate-900">{data.pasien}</h4>
                    </div>
                  </div>

                  <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><DollarSign className="w-16 h-16 text-slate-900" /></div>
                    <p className="text-slate-500 text-sm font-medium mb-1">Estimasi Pendapatan</p>
                    <div className="flex items-end justify-between">
                      <h4 className="text-3xl font-bold text-slate-900">{data.pendapatan}</h4>
                      <span className="flex items-center text-emerald-600 text-sm font-medium"><TrendingUp className="w-4 h-4 mr-1" /> {data.pendapatanTrend}</span>
                    </div>
                  </div>

                  <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><AlertCircle className="w-16 h-16 text-red-600" /></div>
                    <p className="text-slate-500 text-sm font-medium mb-1">Stok Obat Kritis</p>
                    <div className="flex items-end justify-between">
                      <h4 className="text-3xl font-bold text-red-600">{data.stokKritis} Item</h4>
                      <span className="text-red-600 text-sm font-medium">Perlu Restock</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                  <h3 className="text-lg font-medium text-slate-900 mb-6">Tren Kunjungan Pasien</h3>
                  <div className="h-64 flex items-end justify-between space-x-2">
                    {data.chartData.map((val, i) => (
                      <div key={i} className="w-full flex flex-col items-center group">
                        <div className="w-full bg-blue-100 hover:bg-blue-200 rounded-t-sm transition-all relative flex justify-center" style={{ height: `${val}%` }}>
                          <span className="opacity-0 group-hover:opacity-100 absolute -top-8 bg-slate-800 text-white text-xs py-1 px-2 rounded transition-opacity">
                            {val * 10}
                          </span>
                        </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                  <h3 className="text-lg font-medium text-slate-900 mb-6">Aktivitas Terbaru</h3>
                  <div className="space-y-6">
                    {filteredLogs.map((log) => (
                      <div key={log.id} className="flex relative">
                        <div className="absolute top-0 left-4 bottom-[-24px] w-px bg-slate-200 last:hidden"></div>
                        <div className="relative z-10 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mr-4 bg-slate-50 border border-slate-200">
                          {log.type === 'med' && <Activity className="w-4 h-4 text-emerald-600" />}
                          {log.type === 'fin' && <DollarSign className="w-4 h-4 text-yellow-600" />}
                          {log.type === 'inv' && <Package className="w-4 h-4 text-red-600" />}
                          {log.type === 'hr' && <Users className="w-4 h-4 text-blue-600" />}
                        </div>
                        <div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col md:flex-row gap-4 items-center justify-between">
                    <div className="flex items-center w-full md:w-1/2 relative">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-900 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
                    </div>
                    <div className="w-full md:w-auto">
                      <select 
                        ...
                        className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 outline-none"
                      >
          ...
          {activeTab !== 'dashboard' && activeTab !== 'hrd' && activeTab !== 'training' && (
            <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-xl border border-slate-200 shadow-sm">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Modul Sedang Dikembangkan</h3>
